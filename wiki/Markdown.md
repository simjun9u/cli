...
마크다운(Markdown)은 텍스트 기반의 가벼운 마크업 언어로, 일반 텍스트 문서의 서식을 쉽게 지정할 수 있도록 설계되었습니다. 오늘날 마크다운은 기술 문서 작성, 블로그, 위키, 그리고 [[Obsidian]]과 같은 개인 지식 관리 도구에서 널리 사용되고 있습니다.
...
## 3. 마크다운과 Obsidian
[[Obsidian]]은 마크다운 파일(`.md`)을 로컬 파일 시스템에 그대로 저장하는 **Local-first** 지식 관리 도구입니다. GFM을 기본적으로 완벽히 지원하며, 이중 대괄호(`[[링크]]`)를 통한 페이지 간의 위키 스타일 연결과 같은 추가적인 확장 마크업 문법을 지원합니다.
