...
├── Bookmark/          # URL(공식 문서) 기준 정보 수집 및 주기적 웹 동기화 영역
│   ├── targets/       # 사용자가 관리하는 수집 대상 URL 목록 마크다운 저장소
│   └── tracking/      # 동기화 에이전트가 생성/갱신하는 최신 통합 리포트 저장소
...
### 2-4. Sync (북마크 동기화)
* **흐름**: 동기화 에이전트가 `Bookmark/targets/` 폴더 내의 마크다운 파일을 정기 검색하여 변경된 웹페이지 정보를 크롤링함.
* **동작**: 수집 및 요약 결과를 바탕으로 `Bookmark/tracking/` 폴더에 통합 리포트를 생성하고, 타깃 파일의 동기화 상태(`last_synced`)를 업데이트함.
